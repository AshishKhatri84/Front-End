  ...
  return (
    <div className="container py-6 max-w-5xl">
      <div className="flex items-center justify-between mb-6">
        ...
        {unreadCount > 0 && (
          <Button variant="outline" onClick={markAllAsRead} className="text-green-500 border-green-500 hover:bg-green-50">
            Mark all as read
          </Button>
        )}
      </div>
      ...
      <Tabs defaultValue="all" className="space-y-4">
        <TabsList>
          <TabsTrigger value="all" className="text-blue-500 hover:bg-blue-50">
            All
            {unreadCount > 0 && (
          ...
          <TabsTrigger value="unread" className="text-blue-500 hover:bg-blue-50">Unread</TabsTrigger>
          <TabsTrigger value="mentions" className="text-blue-500 hover:bg-blue-50">Mentions</TabsTrigger>
        </TabsList>
              ...
              <div className="divide-y">
                {notifications.length > 0 ? (
                  notifications.map((notification) => (
                    <div 
                      key={notification.id} 
                      className={`flex items-start p-4 gap-3 hover:bg-muted/50 transition-colors ${notification.read ? '' : 'bg-green-50'}`}
                    >
                      <div className={`p-2 rounded-full ${notification.read ? 'bg-muted' : 'bg-green-100'}`}>
                ...
                {notifications.filter(n => !n.read).length > 0 ? (
                  notifications
                    ...
                    .map((notification) => (
                      <div 
                        key={notification.id} 
                        className="flex items-start p-4 gap-3 bg-green-50 hover:bg-muted/50 transition-colors"
                      >
